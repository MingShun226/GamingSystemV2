...
register_wager_user function:
  ...
  BEGIN
      ...
      UPDATE wager_wave_users new_user
      SET referral_code = user_referral_code,
          updated_at = NOW()
      WHERE new_user.id = new_user_id;

      -- Handle referral bonus if there's a referrer
      IF referred_by_id IS NOT NULL THEN
          -- Get referrer username
          SELECT referrer.username
          INTO referrer_username
          FROM wager_wave_users referrer
          WHERE referrer.id = referred_by_id;

          IF referrer_username IS NOT NULL THEN
              -- Award registration bonus to referrer
              UPDATE wager_wave_users referrer_user
              SET points = referrer_user.points + registration_bonus,
                  last_activity = NOW(),
                  updated_at = NOW()
              WHERE referrer_user.id = referred_by_id;
